...
const string AjiVersion        = "v0.6.0";                 // the-database/animejanai-inference. Since v0.4.0: 4:4:4 input on both the TensorRT (v0.5.0) and DirectML paths, opset-21 SD preset (opset-23 fell back to the CPU EP), missing-model passthrough (a chain with an absent .onnx is skipped instead of failing), configurable RIFE/upscale order, and a UTF-8 manifest so aji_encode handles Unicode file paths.
